...
class ActivationsHookGemma2:
    ...
    def _register_hooks(self):
        ...
        def queries_hook(q_proj_module, input, output, layer_idx):
            ...
            queries = output.view(bsz, q_len, self.num_attention_heads, self.head_dim)
        ...
        def keys_hook(k_proj_module, input, output, layer_idx):
            ...
            self.activations[layer_idx]['keys'] = repeat_kv(keys, self.num_key_value_groups).transpose(1, 2)
        ...
        def values_hook(v_proj_module, input, output, layer_idx):
            ...
            self.activations[layer_idx]['values'] = repeat_kv(values, self.num_key_value_groups).transpose(1, 2)
...
def hooked_forward_pass(model, input_embeds, attention_mask, correct_labels=None, add_positional_encodings=False, **kwargs):
    ...
    '''
    # --- Checking if past_key_values are provided
    if 'past_key_values' in kwargs and kwargs['past_key_values'] is not None:
        past_key_values = kwargs['past_key_values']
        prefix_seq_len = past_key_values.seq_length # Getting the length of the prefix sequence (this is a custom attribute created in cache_prefix_text)
        seq_len = input_embeds.shape[1]
        cache_position = torch.arange(prefix_seq_len+1, prefix_seq_len + seq_len+1, device=model.device)
        kwargs['cache_position'] = cache_position # Adding cache_position to the kwargs
        #print('cache_position', cache_position)


    loss = None
    hook = ActivationsHookGemma2(model, add_positional_encodings=add_positional_encodings)
    try:
        output, activations = hook(inputs_embeds=input_embeds, attention_mask=attention_mask, **kwargs)
        hook.remove_hooks()
    except Exception as e:
        hook.remove_hooks() # This is to ensure that the hooks are removed even if an exception is raised
        raise e
