...
def embed_tokens(input_tokens, model):
    '''
        ...
        ------------
            ...
            model (LlamaForCausalLM) – the model to be used for the token embedding.
...
def prepare_batch(batch, model, tokenizer, soft_prompt_embeds=None, soft_prompt_location='before', prompt_text_field='text', answer_field=None, verbose=False, add_special_tokens=True):
    ...
    if soft_prompt_location=='after': # <|begin_of_text|> + PROMPT_TOKENS + SOFT_PROMPT + PAD
        if add_special_tokens:
            input_token_ids_unpadded = [
                torch.cat([
                    tokenizer.encode('<|begin_of_text|>', add_special_tokens=False, return_tensors='pt'),
                    prompt_tokens[k],
            ...
            input_embeds_unpadded = [
                torch.cat([
                    embed_tokens(tokenizer.encode('<|begin_of_text|>', add_special_tokens=False, return_tensors='pt'), model),
    ...
    if soft_prompt_location=='before': # <|begin_of_text|> + SOFT_PROMPT + PROMPT_TOKENS + PAD
        if add_special_tokens:
            input_token_ids_unpadded = [
                torch.cat([
                    tokenizer.encode('<|begin_of_text|>', add_special_tokens=False, return_tensors='pt'),
